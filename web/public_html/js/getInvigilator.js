function getInvigilatorFunction() {
  ...
  // console.log(aboutMe);
  $.when(Posthandler("/route/getInvigilator", aboutMe, true)).done(function(res) {
    if (res.resCode == 'OK') {
      console.log("getInvigilator Responce>>>",res);
      var arr = {};
      arr = res.results;
      $('#tablecontainerDiv').html("<section class='content'><div class='boxC'><table id='aboutMEeTable' class='cell-border display aboutMEeTable_header'  cellspacing=0 width=100%><thead class='leaveTable_header'><tr><th>Name</th><th>Email</th><th>Contact No</th><th>Father</th><th>Address</th><th>Experience</th><th>Date Of Birth</th><th>Edit Information</th></tr></thead><tbody class='aboutMeTable_body'></tbody></table></div></section>");
      $.each(arr, function(i, arr) {
        var body = "<tr>";
        body += "<td>" + arr["name"] + "</td>";
        body += "<td>" + arr["email"] + "</td>";
        body += "<td>" + arr["mobile"] + "</td>";
        body += "<td>" + arr["father"] + "</td>";
  ...
  return $(name).DataTable({
    ...
    buttons: [{
      ...
      },
      {
        extend: 'pdf',
        text: 'Save As PDF',
        title: 'All_Selected_Invigilator_PDF_List.',
        exportOptions: {
          columns: ':visible:not(.not-exported)',
          columns: [0,1, 2, 3, 4, 5, 6],
          modifier: {
